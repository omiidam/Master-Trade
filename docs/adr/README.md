...
| [0052](./ADR-0052-one-process-machine-two-implementations.md)       | **One process machine, two implementations**: `@shared/desktop/process` declares the nine states, the transitions, the report and every deadline; `sidecar.rs` mirrors them literally and `src/desktop/` implements them against a real `node:child_process` port, while `desktop:verify` fails the build if either description drifts (`process-state`, `process.policy`, `protocol`) — because a phase whose subject is process reliability cannot verify itself from source text; **readiness is process running + health answered + this launch's credential**, never a spawn; **a crash is watched** with bounded backoff and `crashed`/`restarting` kept distinct; **stopping asks first** (real `SIGTERM`, then the bound, then force) and both pipes are drained so the API cannot block on its own log; the Rust supervisor's runtime behaviour is explicitly **unverified here** (TDR-13)                                                                                                                                                                                                                                                                                              | `DEC-DESKTOP-7-PROCESS-SUPERVISION` | Accepted |
| [0053](./ADR-0053-bytes-have-no-owner-and-metadata-has-no-path.md)  | **Bytes have no owner, and metadata has no path**: content is content-addressed under `<app-data>/data/<environment>/files` and takes **no path from any caller** (64 hex characters or nothing), while owner, category, MIME type, size, hash, sensitivity and `created_at` live in the `files` table — so moving the storage root is not a migration and neither layer can answer the other's question; **desktop paths are resolved, never discovered** (a pure function of platform, environment and environment variables, never `process.cwd()`), and the three environments are **disjoint subtrees**; **initialization returns a state** (`paths` \| `database` \| `migrations`) because a corrupted ledger is a screen rather than an exception, with the migration policy unchanged — forward-only, checksummed, refused rather than reset; **one module owns the seam**, writing content before the row, compensating only for content it introduced, and deleting metadata before content because an orphan blob is invisible where a dangling row is a visible failure every time                                                                                                   | `DEC-DESKTOP-8-STORAGE`             | Accepted |
| [0054](./ADR-0054-a-credential-is-a-name-a-port-and-a-lifecycle.md) | **A credential is a name, a port and a lifecycle**: credential names are **declared** (namespace `master-trade/` plus a finite list in `@shared/desktop/secrets`) and a well-shaped but undeclared key is a _policy_ refusal, so a command cannot probe for entries the product does not know about; **one port, three implementations** — the Rust keychain, injected environment in the API process, and an unavailable fallback — with `set`/`delete` **refusing** in the API process because a filesystem- or database-backed secret store is the one implementation that must not exist; **existence without the value** (`secure_store_has`, protocol v3), because the status card's question is about state; **five machine-checked `secrets.*` rules** (namespace agreement, single keyring file, `secure_store_*` naming, no enumeration, declared-and-namespaced) hold the half this environment cannot execute; and **one holder with one lifecycle** — read once at startup with every failure reported rather than thrown, resolved synchronously for the gateway, and released on shutdown, exposing only metadata so a secret cannot reach a log, a bundle or a reasoning context | `DEC-DESKTOP-9-CREDENTIALS`         | Accepted |
...
## Phase 6.3 — where desktop data lives

The database layer was already strong: a schema declared as data, dialect-generated DDL, a ledger
that refuses drift and tampering, and a repository per table. What it had nowhere to do was _live on
a desktop_ — the configured database file is resolved against the working directory, which an
installer upgrade replaces. Phase 6.3 resolved every shell-owned path against the OS application-data
directory instead, as a pure function, with the three environments in disjoint subtrees so a
development launch can never open the database a user's history lives in. It also gave file storage
its first real implementation, and split it along the line the schema already drew: the bytes are
content-addressed and have no owner, the row has an owner and stores no path, and one module owns the
write that touches both — because a stored file is two writes, and which one is left standing after an
interruption is a decision rather than an accident.

Database location, migration lifecycle, path security, the metadata relationship, recovery behaviour
and what remains deferred are in [desktop-storage.md](../desktop-storage.md).

## Phase 6.4 — a credential has a name, a port and a lifecycle

The keychain had worked since Phase 2 and had never had a contract. Any well-shaped name was a legal
entry, so the product could not tell its own credential from a typo; and a `keychain` secret
reference resolved to `null` in the API process _unconditionally_, so a keychain-backed provider key
could not be configured at all — a documented capability with no path to being used. Phase 6.4
declared the credentials, put one port in front of the keychain with an injected-environment
implementation for the process that cannot reach it, added `has` so a screen can render "configured"
without the value crossing the boundary, and gave the value a single holder that is read once,
resolved synchronously and released on shutdown. The Rust half still cannot be executed here, so its
own rules are machine-checked instead — namespace, naming, single keyring file, no enumeration.
