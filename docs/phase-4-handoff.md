...
| B-4 | **5 dev/test advisories** (`vite`/`vitest`/`esbuild`/`vite-node` chain)                                                                                                     | Production scope is **0**. Reachability analysed and judged low in [dependency-audit.md](./dependency-audit.md); needs a scoped Vitest 4 + Vite major upgrade. **Not** applied silently |
